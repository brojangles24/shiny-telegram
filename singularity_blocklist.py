#!/usr/bin/env python3
"""
🚀 Singularity DNS Blocklist Aggregator (v5.8 - Advanced Metrics Edition)
...
async def fetch_list(session: aiohttp.ClientSession, url: str, name: str, logger: ConsoleLogger) -> List[str]:
    ...
    headers = {'User-Agent': 'SingularityDNSBlocklistAggregator/5.8'}
...
def filter_and_prioritize(
    ...
    try:
        tld_lines_req = requests.get(HAGEZI_ABUSED_TLDS, timeout=45, headers={'User-Agent': 'SingularityDNSBlocklistAggregator/5.8'})
...
def generate_markdown_report(
    ...
    report.append(f"# 🛡️ Singularity DNS Blocklist Dashboard (v5.8)")
    ...
    report.append("| Source | Category | Weight | Total Fetched | In Priority List | % List In Priority | Volatility ($\pm \%$) | Color |")
    report.append("| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :--- |")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Singularity DNS Blocklist Aggregator (v5.8)")
    ...
    start = datetime.now()
    logger.info("--- 🚀 Starting Singularity DNS Aggregation (v5.8 - ADVANCED METRICS) ---")
